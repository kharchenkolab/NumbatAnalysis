...
for (sample in samples) {
    ...
    for (smooth in c(TRUE)) {
        ...
        win.size = 25
        KS.cut = 0.1
        out_dir = glue('{home_dir}/paper_data/copykat_out/{sample}_normal')
...
for (sample in samples) {
    ...
    for (smooth in c(TRUE)) {
        ...
        if (smooth) {
            window_length = 101
            out_dir = glue('{home_dir}/inferCNV/{sample}/{ref}/smooth')
        } else {
            window_length = 1
            out_dir = glue('{home_dir}/inferCNV/{sample}/{ref}/no_smooth')
        }
        ...
        infercnv_obj = infercnv::CreateInfercnvObject(
            ...
            delim="\t",
            gene_order_file=glue("{home_dir}/inferCNV/hg38_gene_order.txt"),
...
for (sample in samples) {
    ...
    bulk_tumor = fread(glue('{home_dir}/paper_data/numbat_out/{sample}/bulk_tumor.tsv')) %>% 
        mutate(CHROM = factor(CHROM)) %>%
        as.data.frame()
    segs_consensus = fread(glue('{home_dir}/paper_data/purple_out/{sample}/segs_consensus_dna.tsv')) %>% 
        mutate(CHROM = factor(CHROM)) %>%
        as.data.frame()

    out_dir = glue('{home_dir}/paper_out/cnv_benchmark/{sample}')
